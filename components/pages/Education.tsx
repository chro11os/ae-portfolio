...
export const Education = () => {
  ...
  const activeItem = education.items.find((item) => item.id === selectedId) || education.items[0];

  return (
    <Section className="flex flex-col justify-center px-4 md:px-12 lg:px-24 py-4 md:py-8">
      
      {/* 1. HEADER with Subtle Shadows */}
            ...
            >
              ...
              <div className="absolute right-0 bottom-0 opacity-5 pointer-events-none">
                <span className="font-display font-bold text-[8rem] leading-none text-brand-text">
                  {education.items.findIndex(i => i.id === activeItem.id) + 1}
                </span>
              </div>
